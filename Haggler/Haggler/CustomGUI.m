...
-(UIView*)statePickerKeyboard{
    _stateKeyboard = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 375, 200)];
    _stateKeyboard.backgroundColor = [UIColor colorWithWhite: 0.80 alpha:1];
    _statePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, 375, 200)];
    _statePicker.delegate = self;
    _statePicker.dataSource = self;
    [_stateKeyboard.viewForBaselineLayout addSubview:_statePicker];
    
    _doubletap = [[UITapGestureRecognizer alloc] init];
    [_doubletap setNumberOfTapsRequired:2];
    [_doubletap setNumberOfTouchesRequired:1];
    [_stateKeyboard.viewForBaselineLayout addGestureRecognizer:_doubletap];
    
    return _stateKeyboard;
}
-(NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    return sharedDelegate.listOfStates.count;
}
-(NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView{
    return 1;
}
-(NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    NSString *str = [sharedDelegate.listOfStates objectAtIndex:row];
    return str;
}
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    _stateString = [sharedDelegate.listOfStates objectAtIndex:row];
}
